...
0
|| checktplrefresh('./template/default/member/login.htm', './template/default/common/seccheck.htm', 1410698069, '1', './data/template/1_1_member_login.tpl.php', './template/default', 'member/login')
...
<a href="member.php?mod=<?php echo $_G['setting']['regname'];?>" class="xi2">沒有帳號？<a href="member.php?mod=<?php echo $_G['setting']['regname'];?>"><?php echo $_G['setting']['reglinkname'];?></a></a>
</span>
<?php if(!$secchecklogin2) { ?>
<h3 class="xs2">登錄</h3>
<?php } else { ?>
<h3 class="xs2">請輸入驗證碼後繼續登錄</h3>
<?php } ?>
</div>
<div>
<?php } ?>
...
<?php if(!empty($_GET['infloat'])) { if(!empty($_GET['guestmessage'])) { ?>您需要先登錄才能繼續本操作<?php } elseif($auth) { ?>請補充下面的登錄信息<?php } else { ?>用戶登錄<?php } } ?>
</em>
<span><?php if(!empty($_GET['infloat']) && !isset($_GET['frommessage'])) { ?><a href="javascript:;" class="flbc" onclick="hideWindow('<?php echo $_GET['handlekey'];?>', 0, 1);" title="關閉">關閉</a><?php } ?></span>
</h3>
...
<tr>
<th>
<?php if($this->setting['autoidselect']) { ?><label for="username_<?php echo $loginhash;?>">帳號:</label><?php } else { ?>
...
<table>
<tr>
<th><label for="password3_<?php echo $loginhash;?>">密碼:</label></th>
...
<td class="tipcol"><a href="javascript:;" onclick="display('layer_login_<?php echo $loginhash;?>');display('layer_lostpw_<?php echo $loginhash;?>');" title="找回密碼">找回密碼</a></td>
</tr>
</table>
</div>
...
<option value="0"><?php if($questionexist) { ?>安全提問<?php } else { ?>安全提問(未設置請忽略)<?php } ?></option>
...
<tr>
<th></th>
<td><label for="cookietime_<?php echo $loginhash;?>"><input type="checkbox" class="pc" name="cookietime" id="cookietime_<?php echo $loginhash;?>" tabindex="1" value="2592000" <?php echo $cookietimecheck;?> />自動登錄</label></td>
</tr>
</table>
</div>
...
<td>
<?php if($this->setting['sitemessage']['login'] && empty($_GET['infloat'])) { ?><a href="javascript:;" id="custominfo_login_<?php echo $loginhash;?>" class="y">&nbsp;<img src="<?php echo IMGDIR;?>/info_small.gif" alt="幫助" class="vm" /></a><?php } if(!$this->setting['bbclosed'] && empty($_GET['infloat'])) { ?><a href="javascript:;" onclick="ajaxget('member.php?mod=clearcookies&formhash=<?php echo FORMHASH;?>', 'returnmessage_<?php echo $loginhash;?>', 'returnmessage_<?php echo $loginhash;?>');return false;" title="清除痕跡" class="y">清除痕跡</a><?php } ?>
</td>
</tr>
</table>
</div>
...
<h3 class="flb">
<em id="returnmessage3_<?php echo $loginhash;?>">找回密碼</em>
<span><?php if(!empty($_GET['infloat']) && !isset($_GET['frommessage'])) { ?><a href="javascript:;" class="flbc" onclick="hideWindow('login')" title="關閉">關閉</a><?php } ?></span>
</h3>
